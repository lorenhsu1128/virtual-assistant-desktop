...
| 最低系統需求 | Windows 10 1903+、WebView2 Runtime、支援 WebGL 2.0 之顯示卡 |
...
- **技術：** 使用 MediaPipe Face Landmarker，需驗證在 Tauri WebView2 中可正常存取 getUserMedia。
- **隱私指示：** 攝影機啟用時同樣須有明顯的視覺指示。預設為關閉，由使用者主動啟用。

> ⚠️ **技術風險：** WebView2 對 getUserMedia 的支援需在 MVP 前先做 prototype 驗證。若不可行，備案為透過 Rust 側呼叫原生攝影機 API 並將影像傳遞至前端。

### 2.7 視窗互動系統 (Window Interaction)

#### 2.7.1 視窗感知（Rust 側）

- **實作方式：** 透過 Rust 側呼叫 Windows API（`EnumWindows` + `GetWindowRect` + `GetWindowInfo`），定期輪詢取得所有可見視窗的位置、大小與 Z-order。
- **輪詢頻率：** 每秒 3–5 次。
- **過濾規則：** 排除不可見視窗、最小化視窗、桌寵自身視窗，產生「視窗碰撞體清單」透過 Tauri IPC 傳送至前端。
...
軟體僅允許同時運行一個實例。啟動時透過 Tauri 的 single instance plugin 或系統層級的 named mutex 檢測是否已有實例運行。若偵測到已有實例，新啟動的程序將已運行的實例視窗帶到前景後自行退出。
...
|------|------|
| 框架 | Tauri 2.x (Rust) |
| 優勢 | 執行檔體積極小，記憶體佔用低，內建透明視窗與拖曳 API |
| 注意事項 | 依賴 WebView2 Runtime，需於安裝時偵測並靜默安裝 |

> ⚠️ **備選方案：** 若 Tauri WebView2 在透明視窗 + getUserMedia 場景不穩定，可考慮改用 Electron，以保留未來跨平台擴展的可能性。
...
Rust 側（後端）與 TypeScript 側（前端）透過 Tauri IPC 機制溝通。以下為主要的 command 與 event 定義：

**Commands（前端呼叫 Rust）：**

| Command 名稱 | 說明 | 回傳 |
|--------------|------|------|
| `get_window_list` | 取得當前可見視窗的位置、大小、Z-order 清單 | `Vec<WindowRect>` |
| `set_window_region` | 設定桌寵視窗的裁切區域（遮擋用） | `Result<()>` |
| `scan_animations` | 掃描指定資料夾內的 .vrma 檔案 | `Vec<AnimationEntry>` |
| `read_config` | 讀取 config.json | `Config` |
| `write_config` | 寫入 config.json | `Result<()>` |
| `read_animation_meta` | 讀取 animations.json | `AnimationMeta` |
| `write_animation_meta` | 寫入 animations.json | `Result<()>` |
| `pick_file` | 開啟系統檔案選擇器（VRM / 資料夾） | `Option<String>` |
| `get_microphone_level` | 取得當前麥克風音量 | `f32` |
...
| Event 名稱 | 說明 | 資料 |
|------------|------|------|
| `window_layout_changed` | 桌面視窗佈局發生變化時推送 | `Vec<WindowRect>` |
| `display_changed` | 螢幕解析度或 DPI 變更時推送 | `Vec<DisplayInfo>` |
...
## 7. Windows 平台適配 (Platform Specifics)

### 7.1 DPI 處理

使用 Per-Monitor DPI Aware v2 模式。在不同縮放比例的螢幕間拖動角色時，必須動態重新計算視窗座標與渲染縮放，避免角色大小跳變。
...
使用 `tauri-plugin-store` 將使用者偏好儲存於 `~/.virtual-assistant-desktop/config.json`，包含：
...
- **未預期崩潰：** 透過 Rust panic hook 捕捉，寫入 `~/.virtual-assistant-desktop/crash/`，下次啟動時提示使用者可回報。
...
- **整合測試：** 測試 Tauri 與前端的 IPC 通訊、檔案讀取、設定持久化。
...
- **AI 對話與系統操作：** 整合 LLM API，讓桌寵能與使用者對話。進一步支援透過自然語言指令驅動桌寵執行作業系統內的 CLI 工具（如開啟應用程式、檔案操作、系統查詢等），協助使用者操作電腦。需設計權限控管與指令確認機制，避免誤執行高風險操作。**Tauri 權限預留：** 此功能依賴 Tauri 的 shell plugin 與 scope 設定。架構設計階段應在 `tauri.conf.json` 中預先規劃 shell scope 的白名單策略（allowlist 模式），僅允許執行預定義的安全指令集。高風險操作（如刪除檔案、修改系統設定）需經使用者二次確認彈窗。建議設計三層權限模型：安全指令（自動執行）、一般指令（需確認）、禁止指令（永遠拒絕）。
- **插件系統：** 允許第三方開發動作、表情、互動擴充。
- **更多動畫格式：** 支援 VMD（MMD 格式）、BVH 等其他動畫格式匯入。
- **動畫資料夾即時監控：** 使用 file watcher 即時偵測新增/刪除的 .vrma 檔案，無需手動重新掃描。
- **多角色：** 同時顯示多個桌寵角色。
- **macOS 支援：** 若未來有需求，Tauri 架構可相對容易擴展至 macOS。
...
| 風險項目 | 說明與緩解方案 |
|----------|----------------|
| WebView2 + getUserMedia | WebView2 對攝影機存取的支援需驗證。備案：Rust 側原生攝影機 API。 |
